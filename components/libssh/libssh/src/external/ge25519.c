...
static const ge25519_aff_compress ge25519_base_multiples_affine[425] = {
    #include "ge25519_base.data"
};

void ge25519_decompress(ge25519_aff *t, unsigned long long pos, int offset) {
    for (int i = 0; i < 32; i++) {
        t->x.v[i] = ge25519_base_multiples_affine[5 * pos + offset].x.v[i];
        t->y.v[i] = ge25519_base_multiples_affine[5 * pos + offset].y.v[i];
    }
}
...
static void choose_t(ge25519_aff *t, unsigned long long pos, signed char b)
{
    ...
    ge25519_decompress(t, pos, 0);
    ge25519_aff decompressed[4];
    ge25519_decompress(&decompressed[0], pos, 1);
    ge25519_decompress(&decompressed[1], pos, 2);
    ge25519_decompress(&decompressed[2], pos, 3);
    ge25519_decompress(&decompressed[3], pos, 4);


    cmov_aff(t, &decompressed[0],
             equal(b,1) | equal(b,-1));
    cmov_aff(t, &decompressed[1],
             equal(b,2) | equal(b,-2));
    cmov_aff(t, &decompressed[2],
             equal(b,3) | equal(b,-3));
    cmov_aff(t, &decompressed[3],
             equal(b,-4));
